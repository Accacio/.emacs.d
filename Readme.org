...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key ProjetoFinal (kbd "m")
        (defun main () (interactive)
             (open-file-switch-buffer-or-close "~/git/docsTCC/tex/main.tex")
             )
        )

  (define-key ProjetoFinal (kbd "a")
        (defun apresentacao () (interactive)
             (open-file-switch-buffer-or-close "~/git/docsTCC/presentation/main.tex")
             )
        )

  (define-key ProjetoFinal (kbd "p")
    (defun logica-petriNet () (interactive)
           (open-file-switch-buffer-or-close "~/Dropbox/TCC/org/logica.org")
           )
    )
#+END_SRC
